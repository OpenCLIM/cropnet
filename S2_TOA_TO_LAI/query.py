...
def defineFname(tile, projection):
    #print('Projection: ' + projection)
    if tile is not None:
        if projection is not None:
            logger.error('Only tile or projection should be specified, not both')
        else:
            fname = '( filename:S2?_MSIL1C_????????T??????_N*_R*_T%s* )'%tile
        
    else:
        if projection is not None:
            fname = '( filename:S2?_MSIL1C_????????T??????_N*_R*_T' + projection + '* )'
        else:
...
def queryScihub(wkt = None, 
    ...
    fname       = defineFname(tile, projection)
    ...
    for i in feeds:
        ...
        if (len(urls) == 1) & (tile not in urls[0]):
            ...
            if not os.path.exists(fname):
                ...
                logger.info(fname + ' already exists, skipping')
        else:
            # Download individual files
            lostUrls = []
            for url in urls:
                ...
                createParentDir(sysFname)
                urlFnames += [[url, sysFname]]
                if not os.path.exists(sysFname):
                    url = 'gs://gcp-public-data-sentinel-2/tiles/' + tileDir + '/' + urlFname
                    ...
                    logger.info(sysFname + ' already exists, skipping')
            if len(lostUrls) > 0:
...
def downS2FileScihub(feeds, s2FileDir = './'):
    ...
    # MJB: This doesn't work, so forcing it to run in serial instead
    # only two parallel processes allowed by scihub
    #p = Pool(2)
    #ret = p.map(downloader, urlFnames)
    #p.close()
    #p.join()
    for url in urlFnames:
        ret = downloader(url)
    return urlFnames
...
def downloader(urlFname):
    ...
    if os.path.exists(fname) | os.path.exists(fname.replace('.zip', '.SAFE')):
        ...
        if r.ok:
            ...
            if not os.path.exists(fname):
                logger.error('Download failed, skipping file')
            else:
